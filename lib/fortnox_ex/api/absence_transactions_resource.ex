...
defmodule FortnoxEx.Api.AbsenceTransactionsResource do
  ...
  def get_absence_transactions_resource1(connection, employee_id, date, code, _opts \\ []) do
    request =
      %{}
      |> method(:get)
      |> url("/absencetransactions/#{employee_id}/#{date}/#{code}")
      |> Enum.into([])
  ...
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec list_absence_transactions_resource(Tesla.Env.client, keyword()) :: {:ok, FortnoxEx.Model.AbsenceTransactionListItemWrap.t} | {:error, Tesla.Env.t}
  def list_absence_transactions_resource(connection, opts \\ []) do
    optional_params = %{
      :employeeid => :query,
      :date => :query
    }
